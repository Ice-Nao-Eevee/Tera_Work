...
export async function POST(req: NextRequest) {
  try {
    ...
    const systemPrompt = `Anda adalah Asisten AI "Warkop Betawa", restoran kuliner khas Nusantara yang terkenal dengan sambal uleknya.
Tugas Anda:
...
4. JIKA pertanyaan di luar topik kuliner Warkop Betawa (misal: sains, berita politik, coding, dll), TOLAK DENGAN SOPAN dan sarankan untuk menghubungi pelayan atau WhatsApp kami (+6281234567890).
...
${menuContext}
    ...
    let fallbackReply = `Selamat datang di Warkop Betawa! ☕ Ada yang bisa saya bantu terkait menu hidangan, rekomendasi sambal, atau info pemesanan meja Anda?`;

    const lower = lastUserMessage.toLowerCase();
    if (lower.includes('rekomendasi') || lower.includes('paling enak') || lower.includes('favorit')) {
      fallbackReply = `Rekomendasi terbaik kami hari ini adalah **Nasi Goreng Spesial** (Rp 45.000) dengan bumbu rempah warisan dan sate ayam, serta **Sate Ayam Madura** (Rp 38.000) dengan bumbu kacang gurih manis. Selamat mencoba! ☕`;
      ...
      fallbackReply = `Warkop Betawa buka setiap hari pukul 10:00 - 22:00 WIB. Lokasi kami di Jl. Nusantara No. 14, Jakarta.`;
    }
    ...
    return NextResponse.json(
      { reply: 'Maaf, terjadi kendala koneksi dengan Asisten AI. Silakan tanyakan langsung ke staf pelayan kami ya! ☕' },
      { status: 200 }
    );
  }
}
