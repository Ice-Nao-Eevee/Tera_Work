  ...
  if (items.length === 0) {
    return (
      <main className="min-h-[calc(100vh-160px)] flex flex-col items-center justify-center p-6 text-center max-w-md mx-auto">
        <div className="w-20 h-20 rounded-full bg-[#f3e8d6] border border-[#d4bc8c] text-[#b45309] flex items-center justify-center mb-6">
          <ShoppingBag className="w-10 h-10 stroke-[1.8]" />
        </div>
        <h1 className="font-serif italic font-bold text-3xl text-[#b45309] mb-2">
          Keranjang Kosong
        </h1>
        ...
        <Link
          href="/menu"
          className="px-8 py-3.5 bg-[#b45309] hover:bg-[#631c1c] text-white font-medium text-sm rounded-full shadow-md transition-colors"
        >
          Lihat Menu Utama
        </Link>
      </main>
          ...
          <div className="space-y-4">
            {items.map((ci) => {
              ...
              return (
                <div
                  key={ci.id}
                  className="bg-white rounded-3xl p-5 border border-[#d4bc8c] shadow-card flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-all"
                >
                  <div className="flex items-center gap-4">
                    {/* Item Thumbnail */}
                    <div className="relative w-20 h-20 rounded-2xl overflow-hidden bg-[#f3e8d6] shrink-0 border border-[#d4bc8c]">
                      <Image
                    ...
                    <div>
                      ...
                      <div className="font-bold text-sm text-[#b45309] mt-1 sm:hidden">
                        {formatRupiah(ci.lineTotal)}
                      </div>
                    </div>
                  </div>

                  <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-6 pt-2 sm:pt-0 border-t sm:border-t-0 border-[#f3e8d6]">
                    {/* Stepper */}
                    <div className="flex items-center bg-[#f3e8d6] border border-[#d4bc8c] rounded-full p-1">
                      <button
                        onClick={() => updateCartQty(ci.id, -1)}
                        className="w-7 h-7 rounded-full bg-white text-[#b45309] flex items-center justify-center hover:bg-[#ebdbb7] transition-colors"
                        aria-label="Kurangi"
                      >
                        <Minus className="w-3.5 h-3.5 stroke-[2.5]" />
                      </button>
                      <span className="w-8 text-center font-bold text-[#b45309] text-sm">
                        {ci.qty}
                      </span>
                      <button
                        onClick={() => updateCartQty(ci.id, 1)}
                        className="w-7 h-7 rounded-full bg-[#b45309] text-white flex items-center justify-center hover:bg-[#631c1c] transition-colors"
                        aria-label="Tambah"
                      >
                    ...
                    {/* Price Desktop */}
                    <div className="hidden sm:block font-bold text-base text-[#b45309] min-w-[100px] text-right">
                      {formatRupiah(ci.lineTotal)}
                    </div>
                    ...
                    <button
                      ...
                      className="text-xs font-semibold text-[#8c5950] hover:text-[#b45309] hover:underline"
                    >
                      Hapus
                    </button>
                  </div>
        ...
        <div className="lg:col-span-4 sticky top-24">
          <div className="bg-white rounded-3xl p-6 md:p-8 border border-[#d4bc8c] shadow-card space-y-6">
            ...
            <button
              ...
              className="w-full py-4 bg-[#b45309] hover:bg-[#631c1c] text-white font-medium text-base rounded-full shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2"
            >
